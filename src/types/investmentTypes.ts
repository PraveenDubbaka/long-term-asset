...
export interface DividendRow {
  ...
  avgFxRate: number;
  totalDivCAD: number;
  whtCAD?: number;     // withholding tax in CAD (from income/expense register)
  notes?: string;
}
...
export interface InvAJE {
  ...
  rationale: string;
  lines: InvAJELine[];
  type: 'Correcting' | 'Reclassification' | 'Accrual' | 'Disposition' | 'Fair Value Adj' | 'Tax' | 'Income/Expense';
  confidence: 'High' | 'Medium' | 'Low';
  notes?: string;
  status: 'Draft' | 'Approved' | 'Posted';
  wpRef?: string;
  source?: 'manual' | 'auto';   // auto = derived from Income & Expenses register
  sourceRowId?: string;          // references IncomeExpenseRow.id
  deleted?: boolean;
  deletedAt?: string;
}
...
export interface IncomeExpenseRow {
  ...
  acbImpact?: boolean;
  notes?: string;
  ajeStatus?: 'none' | 'draft' | 'approved' | 'posted';
}

// ── Broker Reconciliation ─────────────────────────────────────────────────────
export interface BrokerReconPosition {
  id: string;
  security: string;
  ticker: string;
  broker: string;
  ...
  currency: InvCurrency;
  unitsPerWAC: number;
  unitsPerStatement: number;
  unitVariance: number;
  costPerWAC: number;
  costPerStatement: number;
  costVariance: number;
  fmvPerWAC: number;
  fmvPerStatement: number;
  fmvVariance: number;
}

export interface BrokerCashAccount {
  id: string;
  broker: string;
  acctLast4: string;
  currency: InvCurrency;
  openingBalance: number;
  closingPerBooks: number;
  closingPerStatement: number;
  variance: number;
}

// ── Flags & Review ────────────────────────────────────────────────────────────
export type FlagPriority = 'HIGH' | 'MEDIUM' | 'LOW' | 'INFO';
export type FlagStatus   = 'Open' | 'Resolved' | 'N/A';
