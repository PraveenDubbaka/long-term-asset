                                      ...
                                      const handlePriorFile = (f: File) => {
                                        ...
                                        setTimeout(() => {
                                          ...
                                          const rows: LtDebtReviewRow[] = [
                                            { id: uid(), sourceFile: f.name, locked: true, name: "Promissory Note 1", lender: "BDC Capital Inc.", type: "", currency: "", originalPrincipal: "400,000.00", currentBalance: "356,687.18", rate: "4.000", interestType: "", startDate: "2024-10-01", maturityDate: "2032-09-30", firstPaymentDate: "2024-10-31", monthlyPayment: "4,875.49", fxRate: "", paymentFrequency: "Monthly", paymentType: "", dayCount: "", compounding: "", ioPeriod: "", balloonAmt: "", collateral: "General security agreement over all present and after-acquired personal property", status: "Active", glPrincipal: "", tbLoanAccount: "", tbInterestAccount: "", openingBalance: "0.00", interestPaid: "15,193.06", principalPaid: "43,312.82" },
                                            { id: uid(), sourceFile: f.name, locked: true, name: "Promissory Note 2", lender: "RBC Royal Bank", type: "", currency: "", originalPrincipal: "400,000.00", currentBalance: "356,687.18", rate: "4.000", interestType: "", startDate: "2024-10-01", maturityDate: "2032-09-30", firstPaymentDate: "2024-10-31", monthlyPayment: "4,875.49", fxRate: "", paymentFrequency: "Monthly", paymentType: "", dayCount: "", compounding: "", ioPeriod: "", balloonAmt: "", collateral: "First charge over real property and improvements", status: "Active", glPrincipal: "", tbLoanAccount: "", tbInterestAccount: "", openingBalance: "0.00", interestPaid: "15,193.06", principalPaid: "43,312.82" },
                                          ];
                                          setLtPriorRows(rows);
                                              ...
                                              <button
                                                ...
                                                onClick={() => {
                                                  ...
                                                  const newLoans: Loan[] = rows.map(row => ({
                                                    ...
                                                    dayCountBasis: (row.dayCount || "") as DayCountBasis,
                                                  ...
                                                  const computedLoans = newLoans.map(loan => {
                                                    ...
                                                    const _startMs = loan.startDate ? new Date(loan.startDate).getTime() : firstPmt.getTime();
                                                    const _actualDays1 = Math.round((firstPmt.getTime() - _startMs) / 86400000);
                                                    let _isFirst = true;
                                                    while (d <= fyEnd && bal > 0.01) {
                                                      const int = (_isFirst && _actualDays1 > 0) ? bal * (loan.rate / 100) * _actualDays1 / 365 : bal * monthlyRate;
                                                      _isFirst = false;
                                                      const prin = Math.min(payment - int, bal); const end = bal - prin;
                                                    ...
                                                    const finalClosingBal  = bal;
                                                    const finalInterest    = totalInterest;
                                                    const finalPrincipal   = totalPrincipal;
                                                    let curPortion = 0;
                                                      ...
                                                      {ltReviewRows.map((row, ri) => {
                                                        ...
                                                        return (
                                                          <tr key={row.id} className={`border-b border-border/40 ${ri % 2 === 1 ? "bg-muted/10" : ""}`}>
                                                            {/* Loan Name */}
                                                            <td className="px-1.5 py-1 min-w-[200px]">
                                                              <textarea rows={1} value={row.name} onChange={e => updateLtRow(row.id,"name",e.target.value)} onInput={e => { const t = e.currentTarget; t.style.height = "auto"; t.style.height = `${t.scrollHeight}px`; }} ref={el => { if (el) { el.style.height = "auto"; el.style.height = `${el.scrollHeight}px`; } }} className={cn(fc("name",true).replace("h-6",""),"w-48 resize-none overflow-hidden")} placeholder="Loan name" />
                                                            </td>
                                                            {/* Lender */}
                                                            <td className="px-1.5 py-1 min-w-[170px]">
                                                              <textarea rows={1} value={row.lender} onChange={e => updateLtRow(row.id,"lender",e.target.value)} onInput={e => { const t = e.currentTarget; t.style.height = "auto"; t.style.height = `${t.scrollHeight}px`; }} ref={el => { if (el) { el.style.height = "auto"; el.style.height = `${el.scrollHeight}px`; } }} className={cn(fc("lender",true).replace("h-6",""),"w-44 resize-none overflow-hidden")} placeholder="Lender" />
                                                            </td>
                                                            {/* Current Collateral */}
                                                            <td className="px-1.5 py-1 min-w-[200px]">
                                                              <textarea rows={1} value={row.collateral} onChange={e => updateLtRow(row.id,"collateral",e.target.value)} onInput={e => { const t = e.currentTarget; t.style.height = "auto"; t.style.height = `${t.scrollHeight}px`; }} ref={el => { if (el) { el.style.height = "auto"; el.style.height = `${el.scrollHeight}px`; } }} className={cn(fc("collateral",false).replace("h-6",""),"w-48 resize-none overflow-hidden")} placeholder="e.g. Real property" />
                                                            </td>
                                                            {/* Type */}
                                          ...
                                          <table className="w-full text-base" style={{ minWidth: 2400 }}>
                                            ...
                                            <tbody>
                                              {addMoreLtRows.map((row, ri) => {
                                                ...
                                                return (
                                                  <tr key={row.id} className={`border-b border-border/40 ${ri % 2 === 1 ? "bg-muted/10" : ""}`}>
                                                    <td className="px-1.5 py-1 min-w-[200px]"><textarea rows={1} value={row.name} onChange={e => updateAddMoreRow(row.id,"name",e.target.value)} onInput={e => { const t = e.currentTarget; t.style.height = "auto"; t.style.height = `${t.scrollHeight}px`; }} ref={el => { if (el) { el.style.height = "auto"; el.style.height = `${el.scrollHeight}px`; } }} className={cn(fc2("name",true).replace("h-6",""),"w-48 resize-none overflow-hidden")} placeholder="Loan name" /></td>
                                                    <td className="px-1.5 py-1 min-w-[170px]"><textarea rows={1} value={row.lender} onChange={e => updateAddMoreRow(row.id,"lender",e.target.value)} onInput={e => { const t = e.currentTarget; t.style.height = "auto"; t.style.height = `${t.scrollHeight}px`; }} ref={el => { if (el) { el.style.height = "auto"; el.style.height = `${el.scrollHeight}px`; } }} className={cn(fc2("lender",true).replace("h-6",""),"w-44 resize-none overflow-hidden")} placeholder="Lender" /></td>
