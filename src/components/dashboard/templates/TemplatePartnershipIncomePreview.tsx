...
const TemplatePartnershipIncomePreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      >
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}>
              <span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Income</span>
            </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              {[
                { label: "Professional Fees", col1: "185,000", col2: "162,000" },
                { label: "Consulting Income", col1: "42,000", col2: "35,000" },
                { label: "Other Revenue", col1: "3,500", col2: "2,800" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("230,500", false, true)}</td>
                <td className={cellStyle}>{fv("199,800", false, true)}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              {/* Expenses */}
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Expenses</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Salaries and Wages", col1: "72,000", col2: "65,000" },
                { label: "Rent", col1: "24,000", col2: "24,000" },
                { label: "Office and Administrative", col1: "8,500", col2: "7,200" },
                { label: "Professional Development", col1: "4,800", col2: "3,600" },
                { label: "Insurance", col1: "6,200", col2: "5,800" },
                { label: "Amortization (Note 3)", col1: "10,000", col2: "9,500" },
                { label: "Interest on Long-Term Debt", col1: "3,500", col2: "4,200" },
                { label: "Other Expenses", col1: "1,500", col2: "1,000" },
              ].map((r, i, arr) => (
                <tr key={`exp-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={`${labelStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>Total Expenses</td>
                <td className={`${cellStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>{fv("130,500", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop}`}>{fv("120,300", false, true)}</td>
              </tr>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("79,500", false, true)}</td>
              </tr>

              <tr><td style={{ height: 8 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              {/* Allocation to Partners */}
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Allocation of Net Income:</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Partner A (60%)", col1: "60,000", col2: "47,700" },
                { label: "Partner B (40%)", col1: "40,000", col2: "31,800" },
              ].map((r, i, arr) => (
                <tr key={`alloc-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
...
export default TemplatePartnershipIncomePreview;
