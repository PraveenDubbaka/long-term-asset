...
const TemplateSoleTaxIncomePreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      >
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              {/* Revenue */}
              <tr><td className={labelStyle} style={{ fontWeight: 700, paddingTop: 8 }}>Revenue (T2125 Line Items)</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Gross Sales / Commissions / Fees", col1: "170,000", col2: "140,500" },
                { label: "Less: Returns, Allowances, Discounts", col1: "(2,500)", col2: "(1,800)" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}>Net Revenue</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("167,500", false, true)}</td>
                <td className={cellStyle}>{fv("138,700", false, true)}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              {/* Cost of Goods Sold */}
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Cost of Goods Sold</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Opening Inventory", col1: "6,900", col2: "5,400" },
                { label: "Purchases (Net of Personal Portion)", col1: "60,500", col2: "49,800" },
                { label: "Less: Closing Inventory", col1: "(8,200)", col2: "(6,900)" },
              ].map((r, i, arr) => (
                <tr key={`cogs-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}>Cost of Goods Sold</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("59,200", false, true)}</td>
                <td className={cellStyle}>{fv("48,300", false, true)}</td>
              </tr>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>{fv("108,300", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop}`}>{fv("90,400", false, true)}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              {/* Tax-Deductible Expenses */}
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Tax-Deductible Expenses</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Wages and Benefits (Deductible)", col1: "32,000", col2: "28,000" },
                { label: "Rent (Business Use %)", col1: "18,000", col2: "18,000" },
                { label: "Utilities (Business Portion)", col1: "3,600", col2: "3,100" },
                { label: "Office Expenses", col1: "3,200", col2: "2,800" },
                { label: "Insurance (Business Portion)", col1: "3,600", col2: "3,400" },
                { label: "Advertising and Promotion", col1: "4,800", col2: "3,500" },
                { label: "Motor Vehicle Expenses (Business Use %)", col1: "3,200", col2: "2,900" },
                { label: "Capital Cost Allowance (Note 3)", col1: "7,600", col2: "6,200" },
                { label: "Interest (Business Portion)", col1: "2,400", col2: "3,000" },
                { label: "Professional Fees", col1: "2,800", col2: "2,500" },
              ].map((r, i, arr) => (
                <tr key={`exp-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={`${labelStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>Total Tax-Deductible Expenses</td>
                <td className={`${cellStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>{fv("81,200", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop}`}>{fv("73,400", false, true)}</td>
              </tr>

              <tr><td style={{ height: 8 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Net Business Income (Loss) — Income Tax Basis</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("27,100", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("17,000", false, true)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default TemplateSoleTaxIncomePreview;
