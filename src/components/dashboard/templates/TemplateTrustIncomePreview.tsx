...
const TemplateTrustIncomePreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      <div ref={pageRef} className="bg-white rounded-sm border border-border" style={{ width: "842px", minHeight: "1191px", boxShadow: "0 4px 24px hsl(220 20% 10% / 0.08), 0 1px 4px hsl(220 20% 10% / 0.04)", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily }}>
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}><span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Trust Income and Expenses</span></div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              <tr><td className={labelStyle} style={{ fontWeight: 700, paddingTop: 8 }}>Trust Income</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Interest Income", col1: "18,500", col2: "16,200" },
                { label: "Dividend Income — Eligible", col1: "12,800", col2: "11,500" },
                { label: "Dividend Income — Non-Eligible", col1: "3,200", col2: "2,800" },
                { label: "Rental Income (Note 5)", col1: "36,000", col2: "34,000" },
                { label: "Capital Gains Realized", col1: "22,000", col2: "15,000" },
              ...
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Total Trust Income</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fv("94,000", false, true)}</td><td className={cellStyle}>{fv("80,700", false, true)}</td>{manualCell()}</tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Trust Expenses</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Trustee Fees", col1: "8,000", col2: "7,500" },
                { label: "Legal and Accounting Fees", col1: "5,500", col2: "5,000" },
                { label: "Investment Management Fees", col1: "4,200", col2: "3,800" },
                ...
                { label: "Amortization (Note 4)", col1: "8,000", col2: "8,000" },
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>{fv("41,400", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop}`}>{fv("37,800", false, true)}</td>
              </tr>

              <tr><td style={{ height: 8 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr>
                <td className={`${labelStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>Trust Income Before Distributions and Taxes</td>
                <td className={`${cellStyle} ${thickBorderTop}`} style={{ fontWeight: 700 }}>{fv("52,600", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop}`}>{fv("42,900", false, true)}</td>
              </tr>
              <tr>
                <td className={labelStyle} style={{ paddingLeft: 16 }}>Less: Distributions to Beneficiaries (Note 6)</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("(40,000)")}</td>
                <td className={cellStyle}>{fvRow("(35,000)")}</td>
              </tr>
              <tr>
                <td className={labelStyle} style={{ paddingLeft: 16 }}>Less: Income Taxes (Note 7)</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("(5,000)")}</td>
                <td className={cellStyle}>{fvRow("(3,500)")}</td>
              </tr>

              <tr><td style={{ height: 4 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Net Trust Income Retained</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("7,600", false, true)}</td>
...
export default TemplateTrustIncomePreview;
