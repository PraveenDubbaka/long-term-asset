...
const TemplateSCorpTaxBalanceSheetPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      <div ref={pageRef} className="bg-white rounded-sm border border-border" style={{ width: "842px", minHeight: "1191px", boxShadow: "0 4px 24px hsl(220 20% 10% / 0.08), 0 1px 4px hsl(220 20% 10% / 0.04)", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily }}>
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}><span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Assets, Liabilities and Stockholders' Equity — Income Tax Basis</span></div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              {[
                ...
                { label: "Accounts Receivable (Tax Basis)", col1: "210,000", col2: "185,000" },
                { label: "Inventories (Tax Basis — FIFO)", col1: "145,000", col2: "130,000" },
                { label: "Prepaid Expenses (Tax Deductible Portion)", col1: "8,000", col2: "7,000" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("458,000", false, true)}</td>
                <td className={cellStyle}>{fv("400,000", false, true)}</td>
              </tr>
              ...
              {[
                ...
                { label: "Section 179 / Bonus Depreciation Assets", col1: "48,000", col2: "35,000" },
                { label: "Less: Accumulated Tax Depreciation (MACRS)", col1: "(195,000)", col2: "(148,000)" },
              ].map((r, i, arr) => (
                <tr key={`ppe-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 32 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={labelStyle} style={{ paddingLeft: 24, fontWeight: 700 }}>Net Property and Equipment (Tax Basis)</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("433,000", false, true)}</td>
                <td className={cellStyle}>{fv("447,000", false, true)}</td>
              </tr>
              ...
              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>TOTAL ASSETS (TAX BASIS)</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("909,000", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("862,000", false, true)}</td>
              </tr>
              ...
              {[
                ...
                { label: "Payroll Taxes Payable", col1: "8,000", col2: "7,000" },
              ...
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Stockholders' Equity (Tax Basis)</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Common Stock, $1 Par Value", col1: "50,000", col2: "50,000" },
                { label: "Additional Paid-in Capital", col1: "125,000", col2: "125,000" },
                { label: "Retained Earnings (Tax Basis)", col1: "393,000", col2: "348,000" },
              ...
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}>Total Stockholders' Equity (Tax Basis)</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("558,000", false, true)}</td>
                <td className={cellStyle}>{fv("513,000", false, true)}</td>
              </tr>
...
export default TemplateSCorpTaxBalanceSheetPreview;
