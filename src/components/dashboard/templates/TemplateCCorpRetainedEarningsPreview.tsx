...
const TemplateCCorpRetainedEarningsPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      >
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}>
              <span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Retained Earnings</span>
            </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700, paddingTop: 8 }}>Retained Earnings, Beginning of Year</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("474,000")}</td>
                <td className={cellStyle}>{fvRow("394,000")}</td>
              </tr>
              ...
              <tr>
                <td className={`${labelStyle} ${borderBottom}`} style={{ paddingLeft: 16 }}>Net Income for the Year</td>
                <td className={`${cellStyle} ${borderBottom}`} style={{ fontWeight: 700 }}>{fvRow("226,000")}</td>
                <td className={`${cellStyle} ${borderBottom}`}>{fvRow("160,000")}</td>
              </tr>
              <tr>
                <td className={labelStyle}></td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("700,000", false, true)}</td>
                <td className={cellStyle}>{fv("554,000", false, true)}</td>
              </tr>
              ...
              {[
                { label: "Cash Dividends Declared — Common Stock", col1: "150,000", col2: "60,000" },
                { label: "Cash Dividends Declared — Preferred Stock", col1: "30,000", col2: "20,000" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                <td className={labelStyle}></td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("180,000", false, true)}</td>
              ...
              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Retained Earnings, End of Year</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("520,000", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("474,000", false, true)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default TemplateCCorpRetainedEarningsPreview;
