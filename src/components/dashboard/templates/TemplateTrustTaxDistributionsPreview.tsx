...
const TemplateTrustTaxDistributionsPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      <div ref={pageRef} className="bg-white rounded-sm border border-border" style={{ width: "842px", minHeight: "1191px", boxShadow: "0 4px 24px hsl(220 20% 10% / 0.08), 0 1px 4px hsl(220 20% 10% / 0.04)", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily }}>
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}><span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Distributions to Beneficiaries — Income Tax Basis</span></div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            <thead>
              <tr>
                <th className={`${thickBorderTop} ${thickBorderBottom}`} style={{ width: "40%", textAlign: "left", padding: "4px 2px" }}>T3 Slip Allocation</th>
            ...
            <tbody>
              <tr><td className={labelStyle} style={{ fontWeight: 700, paddingTop: 8 }}>Income Designated (T3 Slips)</td><td className={cellStyle}></td><td className={cellStyle}></td><td className={cellStyle}></td><td className={cellStyle}></td></tr>
              {[
                { label: "Interest Income (Box 26)", a: "7,400", b: "5,550", c: "5,550", t: "18,500" },
                { label: "Actual Eligible Dividends (Box 49)", a: "5,120", b: "3,840", c: "3,840", t: "12,800" },
                { label: "Actual Non-Eligible Dividends (Box 32)", a: "1,280", b: "960", c: "960", t: "3,200" },
                { label: "Net Rental Income (Box 20)", a: "4,480", b: "3,360", c: "3,360", t: "11,200" },
                { label: "Taxable Capital Gains (Box 21)", a: "4,400", b: "3,300", c: "3,300", t: "11,000" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}>Total Income Designated per T3 Slips</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("22,680", false, true)}</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("17,010", false, true)}</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("17,010", false, true)}</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("56,700", false, true)}</td>
              </tr>

              <tr><td style={{ height: 8 }}></td><td></td><td></td><td></td><td></td></tr>

              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Distributions Paid / Payable</td><td className={cellStyle}></td><td className={cellStyle}></td><td className={cellStyle}></td><td className={cellStyle}></td></tr>
              {[
                { label: "Cash Distributions", a: "12,000", b: "9,000", c: "9,000", t: "30,000" },
                { label: "In-Kind Distributions (at FMV)", a: "4,000", b: "3,000", c: "3,000", t: "10,000" },
              ].map((r, i, arr) => (
                <tr key={`dist-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`}>{fvRow(r.a)}</td>
          ...
          <div style={{ marginTop: 24, fontSize: baseFontSize * 0.85, color: "#5a5a6e" }}>
            <p style={{ marginBottom: 4 }}>T3 slips reflect the character of income for tax purposes (eligible/non-eligible dividends, interest, taxable capital gains at 50% inclusion, rental income). Gross-up amounts for dividends are not included above — beneficiaries report grossed-up amounts on their personal returns.</p>
            <p>Subsection 104(13) designations and preferred beneficiary elections, if applicable, are documented separately.</p>
          </div>
        </div>
...
export default TemplateTrustTaxDistributionsPreview;
