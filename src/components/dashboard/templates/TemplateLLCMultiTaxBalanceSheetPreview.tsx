...
const TemplateLLCMultiTaxBalanceSheetPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      >
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}>
              <span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Assets, Liabilities, and Members' Capital — Income Tax Basis</span>
            </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              {[
                ...
                { label: "Accounts Receivable (Tax Basis)", col1: "41,200", col2: "33,500" },
                { label: "Inventory (Tax Basis)", col1: "18,500", col2: "15,800" },
                { label: "Prepaid Expenses (Tax Deductible Portion)", col1: "2,400", col2: "2,000" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("114,100", false, true)}</td>
                <td className={cellStyle}>{fv("89,300", false, true)}</td>
              </tr>
              ...
              {[
                { label: "Property and Equipment — at Cost", col1: "145,000", col2: "130,000" },
                { label: "Less: Accumulated CCA", col1: "(50,000)", col2: "(35,000)" },
              ].map((r, i, arr) => (
                <tr key={`ca-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 32 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("95,000", false, true)}</td>
                <td className={cellStyle}>{fv("95,000", false, true)}</td>
              </tr>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("209,100", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("184,300", false, true)}</td>
              </tr>
              ...
              {/* MEMBERS' CAPITAL */}
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Members' Capital — Income Tax Basis</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Member A — Capital Account (Tax Basis)", col1: "62,200", col2: "46,500" },
                { label: "Member B — Capital Account (Tax Basis)", col1: "49,200", col2: "37,300" },
              ].map((r, i, arr) => (
                <tr key={`eq-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}>Total Members' Capital — Income Tax Basis</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("111,400", false, true)}</td>
                <td className={cellStyle}>{fv("83,800", false, true)}</td>
              </tr>
...
export default TemplateLLCMultiTaxBalanceSheetPreview;
