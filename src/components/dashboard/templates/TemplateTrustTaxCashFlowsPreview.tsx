...
const TemplateTrustTaxCashFlowsPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              <tr><td className={labelStyle} style={{ paddingLeft: 16 }}>Net Trust Income Before Distributions — Tax Basis</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("36,700")}</td><td className={cellStyle}>{fvRow("33,300")}</td>{manualCell()}</tr>
              ...
              {[
                { label: "Capital Cost Allowance (CCA)", col1: "15,000", col2: "12,000" },
                { label: "Taxable Capital Gains (50% Inclusion)", col1: "(11,000)", col2: "(7,500)" },
                { label: "Non-Taxable Portion of Capital Gains", col1: "(11,000)", col2: "(7,500)" },
              ].map((r, i) => (
                <tr key={`adj-${i}`}>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("29,900", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("31,800", false, true)}</td>
              </tr>
              ...
              {[
                { label: "Proceeds from Disposition of Investments", col1: "45,000", col2: "30,000" },
                { label: "Purchase of Investments — at ACB", col1: "(63,000)", col2: "(50,000)" },
              ].map((r, i, arr) => (
                <tr key={`inv-${i}`}>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("(18,000)", false, true)}</td>
              ...
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Net Decrease in Cash</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fv("(31,100)", false, true)}</td><td className={cellStyle}>{fv("(24,700)", false, true)}</td>{manualCell()}</tr>
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Cash and Cash Equivalents, Beginning of Year</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("72,400")}</td><td className={cellStyle}>{fvRow("97,100")}</td>{manualCell()}</tr>
              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Cash and Cash Equivalents, End of Year</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("85,200", false, true)}</td>
...
export default TemplateTrustTaxCashFlowsPreview;
