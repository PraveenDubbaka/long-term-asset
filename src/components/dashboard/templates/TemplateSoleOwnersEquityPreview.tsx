...
const TemplateSoleOwnersEquityPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      >
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}>
              <span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Proprietor's Capital</span>
            </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700, paddingTop: 8 }}>Proprietor's Capital, Beginning of Year</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("55,500")}</td>
                <td className={cellStyle}>{fvRow("48,100")}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Add:</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Net Income for the Year", col1: "27,800", col2: "16,400" },
                { label: "Additional Capital Contributions", col1: "5,000", col2: "8,000" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}></td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("88,300", false, true)}</td>
                <td className={cellStyle}>{fv("72,500", false, true)}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Less:</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Drawings", col1: "(16,800)", col2: "(17,000)" },
              ].map((r, i, arr) => (
                <tr key={`ded-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Proprietor's Capital, End of Year</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("71,500", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("55,500", false, true)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default TemplateSoleOwnersEquityPreview;
