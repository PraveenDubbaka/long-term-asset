...
const TemplateCCorpTaxBalanceSheetPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      >
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}>
              <span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Assets, Liabilities, and Stockholders' Equity — Income Tax Basis</span>
            </div>
            <div style={{ fontSize: baseFontSize, fontWeight: 700 }}>
              As of <span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>December 31, 20XX</span>
            </div>
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              {[
                { label: "Cash and Cash Equivalents", col1: "125,000", col2: "98,000" },
                { label: "Accounts Receivable (Tax Basis)", col1: "285,000", col2: "245,000" },
                { label: "Inventories (Tax Basis — FIFO)", col1: "190,000", col2: "175,000" },
                { label: "Prepaid Expenses (Deductible Portion)", col1: "15,000", col2: "12,000" },
              ].map((r, i, arr) => (
                <tr key={i}>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("615,000", false, true)}</td>
                <td className={cellStyle}>{fv("530,000", false, true)}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr><td className={labelStyle} style={{ paddingLeft: 16, fontWeight: 700 }}>Property and Equipment (Tax Basis)</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Land", col1: "150,000", col2: "150,000" },
                { label: "Buildings and Improvements", col1: "480,000", col2: "480,000" },
                { label: "Machinery and Equipment", col1: "320,000", col2: "290,000" },
                { label: "Less: Accumulated Tax Depreciation (MACRS)", col1: "(245,000)", col2: "(185,000)" },
              ].map((r, i, arr) => (
                <tr key={`ppe-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 32 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                <td className={labelStyle} style={{ paddingLeft: 24, fontWeight: 700 }}>Net Property and Equipment</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("705,000", false, true)}</td>
                <td className={cellStyle}>{fv("735,000", false, true)}</td>
              </tr>

              <tr><td style={{ height: 4 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              {[
                { label: "Section 197 Intangibles, Net", col1: "28,000", col2: "35,000" },
                { label: "Other Assets", col1: "12,000", col2: "10,000" },
              ].map((r, i) => (
                <tr key={`oth-${i}`}>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("1,360,000", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("1,310,000", false, true)}</td>
              </tr>
              ...
              {[
                { label: "Accounts Payable", col1: "142,000", col2: "118,000" },
                { label: "Accrued Expenses (Tax Deductible)", col1: "52,000", col2: "45,000" },
                { label: "Federal Income Taxes Payable", col1: "22,000", col2: "18,000" },
                { label: "State Income Taxes Payable", col1: "10,000", col2: "8,000" },
                { label: "Current Portion of Long-Term Debt", col1: "25,000", col2: "25,000" },
              ].map((r, i, arr) => (
                <tr key={`cl-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 32 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("251,000", false, true)}</td>
                <td className={cellStyle}>{fv("214,000", false, true)}</td>
              </tr>

              <tr><td style={{ height: 6 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              {[
                { label: "Long-Term Debt, Net of Current Portion", col1: "200,000", col2: "225,000" },
              ].map((r, i, arr) => (
                <tr key={`ltl-${i}`}>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("451,000", false, true)}</td>
                <td className={cellStyle}>{fv("439,000", false, true)}</td>
              </tr>
              ...
              {[
                { label: "Common Stock, $1 Par Value", col1: "100,000", col2: "100,000" },
                { label: "Additional Paid-in Capital", col1: "350,000", col2: "350,000" },
                { label: "Retained Earnings (Tax Basis)", col1: "479,000", col2: "441,000" },
                { label: "Less: Treasury Stock, at Cost", col1: "(20,000)", col2: "(20,000)" },
              ].map((r, i, arr) => (
                <tr key={`eq-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("909,000", false, true)}</td>
                <td className={cellStyle}>{fv("871,000", false, true)}</td>
              </tr>
...
export default TemplateCCorpTaxBalanceSheetPreview;
