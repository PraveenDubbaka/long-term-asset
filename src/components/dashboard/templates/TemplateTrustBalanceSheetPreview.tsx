...
const TemplateTrustBalanceSheetPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
      ...
      <div ref={pageRef} className="bg-white rounded-sm border border-border" style={{ width: "842px", minHeight: "1191px", boxShadow: "0 4px 24px hsl(220 20% 10% / 0.08), 0 1px 4px hsl(220 20% 10% / 0.04)", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily }}>
        ...
        <div style={{ flex: 1, padding: bodyPadding, fontSize: baseFontSize, color: "#1a1a1a", position: "relative" }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 18 * scale, fontWeight: 700, marginBottom: 2 }}><span style={{ background: isEditMode ? "hsl(220 15% 90%)" : "transparent", padding: "1px 6px", borderRadius: 2 }}>Statement of Trust Assets and Liabilities</span></div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              <tr><td className={labelStyle} style={{ paddingLeft: 16, fontWeight: 700 }}>Investments (Note 3)</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Bonds and Fixed-Income Securities", col1: "250,000", col2: "230,000" },
                { label: "Equity Securities", col1: "180,000", col2: "165,000" },
                { label: "Mutual Funds / Pooled Funds", col1: "120,000", col2: "110,000" },
              ].map((r, i, arr) => (
                <tr key={`inv-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 32 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Total Investments</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fv("550,000", false, true)}</td><td className={cellStyle}>{fv("505,000", false, true)}</td>{manualCell()}</tr>

              <tr><td style={{ height: 4 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr><td className={labelStyle} style={{ paddingLeft: 16, fontWeight: 700 }}>Capital Assets (Note 4)</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Real Property Held in Trust", col1: "320,000", col2: "320,000" },
                { label: "Less: Accumulated Amortization", col1: "(24,000)", col2: "(16,000)" },
              ].map((r, i, arr) => (
                <tr key={`ca-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 32 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Net Capital Assets</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fv("296,000", false, true)}</td><td className={cellStyle}>{fv("304,000", false, true)}</td>{manualCell()}</tr>

              <tr><td style={{ height: 4 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Total Trust Assets</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("943,700", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("891,400", false, true)}</td>
              </tr>
              ...
              {/* TRUST CAPITAL */}
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Trust Capital</td><td className={cellStyle}></td><td className={cellStyle}></td>{emptyManualTd}</tr>
              {[
                { label: "Original Trust Capital (Corpus)", col1: "800,000", col2: "800,000" },
                { label: "Accumulated Trust Income", col1: "118,000", col2: "70,400" },
              ].map((r, i, arr) => (
                <tr key={`eq-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr><td className={labelStyle} style={{ fontWeight: 700 }}>Total Trust Capital</td><td className={cellStyle} style={{ fontWeight: 700 }}>{fv("918,000", false, true)}</td><td className={cellStyle}>{fv("870,400", false, true)}</td>{manualCell()}</tr>

              <tr><td style={{ height: 4 }}></td><td></td><td></td>{showManual && <td></td>}</tr>

              <tr>
                <td className={`${labelStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>Total Liabilities and Trust Capital</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("943,700", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("891,400", false, true)}</td>
              </tr>
...
export default TemplateTrustBalanceSheetPreview;
