...
const TemplatePartnershipTaxCashFlowsPreview = ({ isEditMode = false }: { isEditMode?: boolean }) => {
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: baseFontSize }}>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("99,300")}</td>
                <td className={cellStyle}>{fvRow("78,500")}</td>
              </tr>
              ...
              {[
                { label: "Capital Cost Allowance (CCA)", col1: "12,000", col2: "11,000" },
              ].map((r, i) => (
                <tr key={`adj-${i}`}>
              ...
              {[
                { label: "Accounts Receivable", col1: "(10,700)", col2: "(5,200)" },
                { label: "Prepaid Expenses (Tax Deductible)", col1: "(700)", col2: "400" },
                { label: "Accounts Payable and Accrued Liabilities", col1: "4,300", col2: "3,100" },
                { label: "GST/HST Payable", col1: "1,100", col2: "(800)" },
              ].map((r, i, arr) => (
                <tr key={`wc-${i}`}>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("105,300", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("87,000", false, true)}</td>
              </tr>
              ...
              {[
                { label: "Purchase of Capital Assets", col1: "(10,000)", col2: "(15,000)" },
              ].map((r, i, arr) => (
                <tr key={`inv-${i}`}>
              ...
              {[
                { label: "Partner Capital Contributions", col1: "15,000", col2: "10,000" },
                { label: "Partner Drawings", col1: "(88,000)", col2: "(72,000)" },
                { label: "Repayment of Long-Term Debt", col1: "(8,000)", col2: "(8,000)" },
              ].map((r, i, arr) => (
                <tr key={`fin-${i}`}>
                  <td className={`${labelStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ paddingLeft: 16 }}>{r.label}</td>
                  <td className={`${cellStyle} ${i === arr.length - 1 ? borderBottom : ""}`} style={{ fontWeight: 700 }}>{fvRow(r.col1)}</td>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("(81,000)", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("(70,000)", false, true)}</td>
              </tr>
              ...
              <tr>
                ...
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fv("14,300", false, true)}</td>
                <td className={cellStyle}>{fv("2,000", false, true)}</td>
              </tr>
              <tr>
                <td className={labelStyle} style={{ fontWeight: 700 }}>Cash and Cash Equivalents, Beginning of Year</td>
                <td className={cellStyle} style={{ fontWeight: 700 }}>{fvRow("32,000")}</td>
              ...
              <tr>
                ...
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`} style={{ fontWeight: 700 }}>{fv("46,300", false, true)}</td>
                <td className={`${cellStyle} ${thickBorderTop} ${thickBorderBottom}`}>{fv("32,000", false, true)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default TemplatePartnershipTaxCashFlowsPreview;
