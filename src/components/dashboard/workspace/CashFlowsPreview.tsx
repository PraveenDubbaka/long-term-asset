...
interface CashFlowsPreviewProps {
  isEditMode?: boolean;
  onContentChanged?: (hasChanges: boolean) => void;
}

const CashFlowsPreview = ({ isEditMode = false, onContentChanged }: CashFlowsPreviewProps) => {
  ...
  const insertion = settings.manualColumnInsertions.cashFlows;
    ...
    >
      {showAddButtons && (
        <>
          <AddColumnButton
            position="before"
            onClick={() => addManualColumn("cashFlows", { mode: "at", index: dataIdx, side: "before" })}
          />
          {dataIdx === 1 && (
            <AddColumnButton
              position="after"
              onClick={() => addManualColumn("cashFlows", { mode: "at", index: dataIdx, side: "after" })}
            />
          )}
        </>
      )}
...
export default CashFlowsPreview;
