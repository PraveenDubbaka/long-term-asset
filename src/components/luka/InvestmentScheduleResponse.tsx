                          ...
                          {(["eligibleDividend","capitalDividend","otherIncome","foreignIncome","nonResidentTax"] as const).map((field, fi) => (
                            <td key={field} className={`px-2.5 py-1.5 text-right tabular-nums ${fi < 4 ? "border-r border-border/20" : ""} ${fi === 0 ? "bg-emerald-50/10" : ""}`} style={fi === 0 ? { borderLeft: "3px solid hsl(var(--foreground) / 0.3)" } : {}}>
                              {isEditing ? (
                                ...
                                ? <span className={field === "nonResidentTax" ? "text-red-600" : "text-foreground"}>{fmtCAD(r[field]!)}</span>
                      ...
                      {(["eligibleDividend","capitalDividend","otherIncome","foreignIncome","nonResidentTax"] as const).map((field, fi) => {
                        ...
                        return (
                          <td key={field} className={`px-2.5 py-2 text-right tabular-nums ${fi < 4 ? "border-r border-border/30" : ""}`} style={fi === 0 ? { borderLeft: "3px solid hsl(var(--foreground) / 0.3)" } : {}}>
                            {total > 0
                              ? <span className={field === "nonResidentTax" ? "text-red-600 font-semibold" : "text-foreground font-semibold"}>{fmtCAD(total)}</span>
                              : <span className="text-muted-foreground/40 tabular-nums">0.00</span>}
                          </td>
                        );
                      })}
      ...
      <div className="flex items-center border-b border-border -mx-0">
        <div className="flex items-center gap-0 overflow-x-auto flex-1 min-w-0">
          {TABS.map(({ id, label }) => {
            ...
            const locked = (invMode === "add" && id !== "transactions") || (invMode === "edit" && id !== activeTab);
            return (
              <button
                key={id}
